...
const SKILL_TEMPLATE =
  ...
  '    <div>' +
  '      <input style="display:none" type="checkbox"/><span class="query-identifier ag-icon ag-icon-checkbox-unchecked" style="transform: scale(.7)"></span>' +
...
export default class SkillFilter implements IFilter {
  ...
  public getGui() {
    ...
    RefData.IT_SKILLS.forEach(function(skill, index) {
      ...
      const agCheckBoxIcon = eSpan.querySelector('.query-identifier');
      eCheckbox.addEventListener('click', function() {
        that.model[skill] = eCheckbox.checked;
        if(eCheckbox.checked) {
          agCheckBoxIcon.classList.add("ag-icon-checkbox-checked");
          agCheckBoxIcon.classList.remove("ag-icon-checkbox-unchecked");
        } else {
          agCheckBoxIcon.classList.add("ag-icon-checkbox-unchecked");
          agCheckBoxIcon.classList.remove("ag-icon-checkbox-checked");
        }
